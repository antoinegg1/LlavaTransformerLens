...
class HookedLlava(HookedRootModule):
    ...
    def forward(
        self,
        inputs_embeds: Optional[torch.Tensor] = None,  # [batch, seq_len, d_model]
        ...
        **kwargs 
        ...
        None,
        torch.Tensor,  # logits: [batch, seq_len, d_vocab]
        Loss,
        ...
        for i in range(start_at_layer, stop_at_layer):
            block = self.blocks[i]
            # Retrieve past_key_value for the current layer if available
            past_key_value = past_kv_cache.entries[i] if past_kv_cache is not None else None
            ...
            if past_kv_cache is not None:
                past_kv_cache.entries[i] = block.updated_past_key_value
        if self.cfg.normalization_type is not None:
            hidden_states = self.ln_final(hidden_states)  # [batch, seq_len, d_model]
         # Handle the output
        if return_type is None:
            return None

        logits = self.unembed(hidden_states)  # [batch, seq_len, d_vocab]
        ...
        if tokens is None:
            raise ValueError("tokens must be provided when return_type is 'loss' or 'both'")
    ...
    def generate(
        ...
        with utils.LocallyOverridenDefaults(
            ...
            for _ in tqdm.tqdm(range(max_new_tokens), disable=not verbose):
                ...
                # 应用采样策略
                if do_sample:
                    next_tokens = utils.sample_logits(
                        final_logits,
                        top_k=top_k,
                        top_p=top_p,
                    ...
                    next_tokens = final_logits.argmax(dim=-1).to(device)

                # 处理已完成的序列
                if stop_at_eos:
                    next_tokens[finished_sequences] = eos_token_for_padding
                    is_eos = torch.isin(next_tokens, torch.tensor(stop_tokens).to(device))
                    finished_sequences.logical_or_(is_eos)
